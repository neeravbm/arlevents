...
class NFComm {
	...
	public function WEBCentralizedShoppingCartMembershipOpenInvoiceGet($key)
	{
		$arr = Array('Key'=>$key);
		//var_dump($arr);
		$response = $this->nfxweb->WEBCentralizedShoppingCartMembershipOpenInvoiceGet($arr);
		//var_dump($response);
	
		$retval = $response->WEBCentralizedShoppingCartMembershipOpenInvoiceGetResult;
		return  $retval;
	}
	
	public function WEBCentralizedShoppingCartMembershipOpenInvoiceAdd($cart, $invoice)
	{
		$arr = Array('oCentralizedOrderEntry'=>$cart, 'oOpenInvoice'=>$invoice);
	
		$response = $this->nfxweb->WEBCentralizedShoppingCartMembershipOpenInvoiceAdd($arr);
	
		return $response->WEBCentralizedShoppingCartMembershipOpenInvoiceAddResult;
	}
	...
	/*TBE*****************************************************************/
	public function purchaseCart($cart, $payment, $evt_cct_key, $evt_act_cct_key_ext) {
		try {
		//$val = Array();
		...
		$cart->Invoice->inv_send_email_confirmation = 1;
		$cart->Invoice_DetailCollection->InvoiceDetailStatic->Invoice_Detail->ivd_autoapply = 1;
		

		//$cart->RegistrantCollection->EventsRegistrant->Invoice->inv_send_email_confirmation = 1;
		$cart->RegistrantCollection->Invoice_DetailCollection->InvoiceDetailStatic->Invoice_Detail->ivd_autoapply = 1;

		//custom fields added to handle customization....
		$cart->RegistrantCollection->EventsRegistrant->Event->evt_cct_key = "".$evt_cct_key;
		$cart->RegistrantCollection->EventsRegistrant->Event->evt_act_cct_key_ext = "".$evt_act_cct_key_ext;
		
		$cart->RegistrantCollection->EventsRegistrant->Registrant->reg_cancel_date = "";
		$cart->RegistrantCollection->EventsRegistrant->Registrant->reg_on_wait_list_flag = 0;
		//end custom fields
		...
		$cart->Invoice->Payment_Info->pin_cc_cardholder_name = $payment->getpin_cc_cardholder_name();
		$cart->Invoice->Payment_Info->pin_cc_number = $payment->getpin_cc_number();
		$cart->Invoice->Payment_Info->pin_cc_security_code = $payment->getpin_cc_security_code();
		$cart->Invoice->Payment_Info->pin_cc_expire = $payment->getpin_cc_expire();
		$cart->Invoice->Payment_Info->pin_apm_key = $payment->getpin_apm_key();
		$cart->Invoice->Payment_Info->pin_check_amount = $payment->getpin_check_amount();
		$cart->Invoice->Payment_Info->pin_check_amountSpecified = $payment->getpin_check_amountSpecified();
...
//var_dump($arr);
	...
	//					'nom_key'=>'00000000-0000-0000-0000-000000000000', 
					...
					'Nominee' => '', 
					'Nominated_By' => '', 
					'Committee' => array ('cmt_key'=>$cmt_key), 
					'Committee_Nomination_Status' => array ('nms_key'=>'e6ee3cfe-ab62-45bf-b3d2-5879b0dbb0da'), 
	...
	public function getActiveInvoicesForCustomer($cst_key)
	{
		try {
			$arr = Array('CustomerKey'=>$cst_key);
			$response = $this->nfxweb->WEBCentralizedShoppingCartMembershipOpenInvoiceGetList($arr);
			return $response;
		}
		catch(SoapFault $exception) {
			$this->printDebug($exception);
