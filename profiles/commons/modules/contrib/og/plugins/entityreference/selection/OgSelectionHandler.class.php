  ...
  public function getReferencableEntities($match = NULL, $match_operator = 'CONTAINS', $limit = 0) {
    ...
    if (!empty($results[$entity_type])) {
      ...
      foreach ($results[$entity_type] as $gid => $values) {
        ...
        elseif (!empty($node->nid) && (og_user_access($entity_type, $gid, "update any $node_type content") || ($user->uid == $node->uid && og_user_access($entity_type, $gid, "update own $node_type content")))) {
